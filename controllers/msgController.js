...
exports.addMessage = async (req, res) => {
  try {
      ...
      res &&
        res
          ...
          .json({
            ...
            message: `The Message is created`,
            msgDetails,
          });
    ...
    res &&
      res.status(constants.responseStatusCode.internalServerError).json({
        info: constants.failureCode,
...
exports.getSideBarMessagesById = async (req, res) => {
  try {
    ...
    let messages = [...senderMessages, ...receiverMessages].sort(
    ...
    if (messages) {
      ...
      messages.map((message) => {
        const id = slideMsg.has(message.senderId)
          ? message.senderId
          : slideMsg.has(message.receiverId)
          ? message.receiverId
          : null;
        if (id) {
